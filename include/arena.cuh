...
struct stride_functor {
    ...
    __host__ __device__ size_t operator()(size_t i) const { return i * stride; }
};

template <class T> struct SizedIter {
    T iter;
    size_t size;
    ...
    void shrink(size_t new_size) {
        if (new_size <= size) {
            ...
            std::cout << "EXITING! New size must be same or less than current size! "
                         "Current size: "
                      << size << ", new size: " << new_size << std::endl;
            exit(1);
        }
    }
};

template <bool GPU> struct Arena {
    using vector_type =
        typename std::conditional<GPU, thrust::device_vector<unsigned char>, thrust::host_vector<unsigned char>>::type;
    vector_type buffer;
    Arena(size_t num_bytes) : buffer(num_bytes) {}

    // start specified in bytes
    // stride specified in items
    template <typename TYPE> auto view_start_at_bytes(size_t start, size_t size, size_t stride = 1) {
    ...
    // stride specified in items
    template <typename TYPE> auto view_start_at_items(size_t start, size_t size, size_t stride = 1) {
