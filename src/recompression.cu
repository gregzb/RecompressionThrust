...
namespace Cu
{
    ...
    CurrentInputPos other_pos(CurrentInputPos curr_input_pos)
    {
        return curr_input_pos == CurrentInputPos::TWO ? CurrentInputPos::THREE : CurrentInputPos::TWO;
    }

    InputIterT view_for(Arena &arena, CurrentInputPos curr_input_pos, symbol_t original_size, symbol_t curr_input_size)
    {
        symbol_t mult = curr_input_pos == CurrentInputPos::TWO ? 2 : 3;
        return arena.view_start_at_items<symbol_t>(mult * original_size, curr_input_size);
    }

    // 9
    // new input, new num symbols
    // curr input is expected to be a view at symbol item index 2
    symbol_t bcomp(Arena &arena, InputIterT &curr_input, CurrentInputPos &curr_input_pos, symbol_t original_size, symbol_t num_symbols, RulesIterT rules)
    {
        using std::chrono::duration;
        using std::chrono::duration_cast;
        using std::chrono::high_resolution_clock;
    ...
    size_t total_pcomp_sorted = 0;

    // 6
    // new input, new num symbols
    symbol_t pcomp(Arena &arena, InputIterT &curr_input, CurrentInputPos curr_input_pos, symbol_t original_size, symbol_t num_symbols, RulesIterT rules, int x)
    {
        ...
        auto indices = arena.view_start_at_bytes<symbol_t>(sizeof(uint64_t) * original_size / 2 * 1, assigned_bit.size / 2);
        ...
        if (pairs.size > 0)
        {
            ...
            timeF([&]()
                  { thrust::unique_by_key_copy(offsets.iter, offsets.iter + offsets.size, thrust::make_transform_iterator(pairs.iter, [] __device__(uint64_t item) -> thrust::tuple<symbol_t, symbol_t>
                                                                                                                  { return thrust::make_tuple(item >> 32, item & 0xffffffff); }),
                                       thrust::make_discard_iterator(), rules.iter + num_symbols); return 0; }, "unique by key copy");
    ...
    void recompression(symbol_t alphabet_size, std::vector<symbol_t> input)
    {
        ...
        Arena arena(unit_size * sizeof(symbol_t) * 7);
        ...
        auto rules = arena.view_start_at_bytes<thrust::tuple<symbol_t, symbol_t>>(5 * unit_size * sizeof(symbol_t), input.size());
