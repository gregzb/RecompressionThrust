...
template <bool IS_GPU> struct Thrust {
    ...
    enum class CurrentInputPos { TWO, THREE };
    ...
    static symbol_t bcomp(Arena<IS_GPU> &arena, InputIterT &curr_input, CurrentInputPos &curr_input_pos,
        ...
        if (num_blocks > 0) {
            ...
            thrust::transform_inclusive_scan(
                ...
                [] __host__ __device__(
                    const thrust::tuple<thrust::tuple<uint64_t, symbol_t>, thrust::tuple<uint64_t, symbol_t>> &item)
                    -> symbol_t {
                    bool eq = thrust::get<0>(thrust::get<0>(item)) == thrust::get<0>(thrust::get<1>(item));
                    return (symbol_t)(!eq);
                },
                [] __host__ __device__(symbol_t a, symbol_t b) -> symbol_t { return a + b; });
            ...
            auto offset_iter = thrust::make_transform_iterator(
                offsets.iter,
                [num_symbols] __host__ __device__(const symbol_t &offset) -> symbol_t { return num_symbols + offset; });
    ...
    static symbol_t pcomp(Arena<IS_GPU> &arena, InputIterT &curr_input, CurrentInputPos curr_input_pos,
        ...
        thrust::adjacent_difference(
            ...
            [] __host__ __device__(uint8_t curr, uint8_t prev) -> uint8_t { return curr && !prev; });
        ...
        {
            auto pairs_and_sources_transformed_end =
                thrust::copy_if(iter_adj, iter_adj + assigned_bit.size - 1, assigned_bit.iter + 1,
        ...
        if (pairs.size > 0) {
            ...
            if constexpr (IS_GPU) {
                ...
                auto new_end =
                    thrust::remove_if(curr_input_and_idx, curr_input_and_idx + curr_input.size,
                                      [assigned_bit_raw_ptr, assigned_bit_size] __host__ __device__(
                                          const thrust::tuple<symbol_t, symbol_t> &item) {
    ...
    static Rlslp recompression(symbol_t alphabet_size, iter_t begin, iter_t end) {
        ...
        auto rules_transformed = arena.template view_start_at_items<Rlslp::Rule>(0, rules.size);
        ...
        time_f_print_void(
            [&]() {
                ...
                thrust::transform(rules.iter, rules.iter + rules.size, rules_transformed.iter,
                                  [] __host__ __device__(const thrust::tuple<symbol_t, symbol_t> &rule) {
                                      return Rlslp::Rule{
                                          .pair = {.children = {thrust::get<0>(rule), thrust::get<1>(rule)}}};
                                  });
                thrust::copy(rules_transformed.iter, rules_transformed.iter + rules_transformed.size,
                             rules_vec.begin());
